...
import {Clipboard,Text,StyleSheet,TouchableOpacity,View,Image} from 'react-native';
...
class SharePopup extends Component {
  ...
  render() {
    ...
    let shareOptions = this.props.options;
    return (
      <ShareSheet visible={this.props.isVisible} onCancel={this.onCancel.bind(this)}>

        <TouchableOpacity style={styles.buttonContainer}
                onPress={()=>{
                  this.onCancel();
                  SendAnalytics.sendEvent(shareOptions.title,'facebookShare',shareOptions.link);
                  setTimeout(() => {
                    Share.shareSingle(Object.assign(shareOptions, {
                      "social": "facebook"
                    })).catch((err) => { err && Utils.showAlert('Facebook app not installed.'); });
                  },300);
                }} >
          <Image source={{ uri: FACEBOOK_ICON }} style={styles.barIcon}/>
        ...
        <TouchableOpacity style={styles.buttonContainer}
                onPress={()=>{
                  this.onCancel();
                  SendAnalytics.sendEvent(shareOptions.title,'whatsapp',shareOptions.link);
                  setTimeout(() => {
                    Share.shareSingle(Object.assign(shareOptions, {
                      "social": "whatsapp"
                    })).catch((err) => { err && Utils.showAlert('Whatsapp app not installed.'); });
                  },300);
                }} >
          <Image source={{ uri: WHATSAPP_ICON }} style={styles.barIcon}/>
        ...
        <TouchableOpacity style={styles.buttonContainer}
              onPress={()=>{
                  this.onCancel();
                  SendAnalytics.sendEvent(shareOptions.title,'email',shareOptions.link);
                  setTimeout(() => {
                    Share.shareSingle(Object.assign(shareOptions, {
                      "social": "email"
          ...
          <Image source={{ uri: EMAIL_ICON }} style={styles.barIcon}/>
        ...
        <TouchableOpacity style={styles.buttonContainer}
            onPress={()=>{
              this.onCancel();
              SendAnalytics.sendEvent(shareOptions.title,'link copy',shareOptions.link);
              setTimeout(() => {
                if(typeof shareOptions["url"] !== undefined) {
                  Clipboard.setString(shareOptions["url"]);
                  Utils.showAlert('Link copied.');
                }
              },300)
            }} >
          <Image source={{ uri: CLIPBOARD_ICON }} style={styles.barIcon}/>
        ...
        <TouchableOpacity style={styles.buttonContainer}
          onPress={()=>{
            this.onCancel();
            SendAnalytics.sendEvent(shareOptions.title,'other',shareOptions.link);
            setTimeout(() => {
              Share.open(shareOptions).catch((err) => { err && console.log(err); });
            },300)
          }}>
          <Image source={{ uri: MORE_ICON }} style={styles.barIcon}/>
...
//       Share.shareSingle(Object.assign(shareOptions, {
//         "social": "twitter"
//       })).catch((err) => { err && Utils.showAlert('Twitter app not installed.'); });
//     },300);
//   }}>Twitter</Button>
